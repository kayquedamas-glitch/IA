...
export function initDashboard() {
    // 1. Carrega dados visuais
    renderMissions(); 
    updateStreakUI();
    calculateDaysOnBase(); // <--- AGORA ESTÁ SENDO CHAMADO

    // 2. Conecta botão HTML ao JS
...
export function updateStreakUI() {
    ...
    if (streak > 0) {
        ...
        if (streak < 7) fireIcon.classList.add('text-orange-500', 'drop-shadow-[0_0_5px_rgba(249,115,22,0.5)]');
        else if (streak < 30) fireIcon.classList.add('text-red-600', 'scale-125', 'drop-shadow-[0_0_15px_rgba(220,38,38,0.8)]');
        else fireIcon.classList.add('text-purple-500', 'scale-150', 'drop-shadow-[0_0_25px_rgba(168,85,247,0.8)]');
    } else {
...
function renderMissions() {
    ...
    missions.sort((a, b) => {
        if (a.done === b.done) return (new Date(a.date || 0) - new Date(b.date || 0));
        return a.done ? 1 : -1;
    });
